...
- **index.html** → Estrutura principal da aplicação e integração dos scripts.
- **css/style.css** → Estilos visuais e responsividade.
- **js/routes-data.js** → Dados de rotas e distâncias (objeto global).
- **js/config.js** → Constantes de fatores médios de emissão de CO₂ (objeto global).
- **js/calculator.js** → Lógica de cálculo das emissões (funções globais).
- **js/ui.js** → Manipulação da interface e DOM (funções globais).
- **js/app.js** → Inicialização de eventos e integração da aplicação.
...
| Transporte | Emissão média (kg CO₂/km) |
|------------|---------------------------|
| Carro      | 0.192                     |
| Ônibus     | 0.105                     |
| Trem       | 0.041                     |
...
- Inclusão de mais rotas e meios de transporte.  
- Cálculo de compensação em árvores necessárias para neutralizar emissões.  
- Integração com APIs de mapas para cálculo dinâmico de distâncias.  
- Relatórios gráficos de impacto ambiental.  
